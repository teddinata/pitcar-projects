...
<template>
  <div class="bg-white rounded-lg shadow overflow-hidden h-[calc(100vh-140px)] flex flex-col">
    ...
    <div class="grid grid-cols-8 border-b border-gray-200">
      <!-- Time column header with refresh button -->
    ...
    <!-- SECTION 2: CALENDAR CONTENT GRID -->
    <div class="grid grid-cols-8 flex-grow overflow-y-auto relative">
      ...
      <!-- SECTION 3: DAY COLUMNS WITH TIME SLOTS & ACTIVITIES -->
      <div 
          ...
          <div class="p-3 h-full flex flex-col text-left relative">
            ...
            <div v-if="calculateActivityHeight(activity) > 100" class="text-sm mt-2 flex items-center">
              ...
              >
                <span class="mr-1">
                  <!-- Activity type icon -->
                  <svg v-if="activity.activity_type === 'meeting'" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clip-rule="evenodd" /></svg>
                  <svg v-else-if="activity.activity_type === 'training'" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor"><path d="M10.394 2.08a1 1 0 00-.788 0l-7 3a1 1 0 000 1.84L5.25 8.051a.999.999 0 01.356-.257l4-1.714a1 1 0 11.788 1.838L7.667 9.088l1.94.831a1 1 0 00.787 0l7-3a1 1 0 000-1.838l-7-3zM3.31 9.397L5 10.12v4.102a8.969 8.969 0 00-1.05-.174 1 1 0 01-.89-.89 11.115 11.115 0 01.25-3.762zM9.3 16.573A9.026 9.026 0 007 14.935v-3.957l1.818.78a3 3 0 002.364 0l5.508-2.361a11.026 11.026 0 01.25 3.762 1 1 0 01-.89.89 8.968 8.968 0 00-5.35 2.524 1 1 0 01-1.4 0zM6 18a1 1 0 001-1v-2.065a8.935 8.935 0 00-2-.712V17a1 1 0 001 1z" /></svg>
                  <svg v-else-if="activity.activity_type === 'support'" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-2 0c0 .993-.241 1.929-.668 2.754l-1.524-1.525a3.997 3.997 0 00.078-2.183l1.562-1.562C15.802 8.249 16 9.1 16 10zm-5.165 3.913l1.58 1.58A5.98 5.98 0 0110 16a5.976 5.976 0 01-2.516-.552l1.562-1.562a4.006 4.006 0 001.789.027zm-4.677-2.796a4.002 4.002 0 01-.041-2.08l-.08.08-1.53-1.533A5.98 5.98 0 004 10c0 .954.223 1.856.619 2.657l1.54-1.54zm1.088-6.45A5.974 5.974 0 0110 4c.954 0 1.856.223 2.657.619l-1.54 1.54a4.002 4.002 0 00-2.346.033L7.246 4.668zM12 10a2 2 0 11-4 0 2 2 0 014 0z" clip-rule="evenodd" /></svg>
                  <svg v-else-if="activity.activity_type === 'admin'" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M3 5a2 2 0 012-2h10a2 2 0 012 2v8a2 2 0 01-2 2h-2.22l.123.489.804.804A1 1 0 0113 18H7a1 1 0 01-.707-1.707l.804-.804L7.22 15H5a2 2 0 01-2-2V5zm5.771 7H5V5h10v7H8.771z" clip-rule="evenodd" /></svg>
                  <svg v-else class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" /></svg>
                </span>
            ...
            <div class="bg-white p-3 rounded-md shadow-lg border border-gray-200">
              ...
              <div class="mt-2 space-y-1">
                ...
                <div class="flex items-start" v-if="activity.project">
                  <svg class="h-4 w-4 mr-1.5 text-gray-500 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clip-rule="evenodd" />
                  </svg>
                  <div class="text-xs text-gray-700">
...
/* ---------- SECTION 1: PROPS AND CONFIGURATION ---------- */
...
const formattedWeekDays = computed(() => {
  ...
  return props.weekData.map(day => {
    ...
    // Format day name (e.g., "Min", "Sen", etc.)
    const dayNames = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'];
    const dayName = dayNames[date.getDay()];
    
    // Format date (e.g., "9 Mar")
...
/* ---------- SECTION 3: ACTIVITY POSITIONING AND SIZING ---------- */
...
// Calculate activity vertical position
const calculateActivityPosition = (activity) => {
  if (!activity.time || !activity.time.start) return 0;
  ...
  try {
    ...
    // If activity starts before visible hours, cap it
    if (hours < startHour) return 0;
...
// Support for overlapping activities
const findOverlappingActivities = (activity, allActivities) => {
  if (!activity.time || !activity.time.start || !activity.time.end) return [];
  
  // Convert activity times to minutes for easier comparison
  ...
  return allActivities.filter(a => {
    if (!a.time || !a.time.start || !a.time.end) return false;
    
    const aStart = timeToMinutes(a.time.start);
    const aEnd = timeToMinutes(a.time.end);
    
    // Activities overlap if:
    // 1. a starts before activity ends AND
    // 2. a ends after activity starts
...
// Calculate width for overlapping activities
// Calculate width for overlapping activities
const calculateActivityWidth = (activity, dayActivities) => {
  ...
  // If not overlapping, use full width
  if (overlappingActivities.length <= 1) {
    return 'calc(100% - 8px)';
  }
  
  // Divide width based on number of overlapping activities
  const widthPercentage = 100 / overlappingActivities.length;
  return `calc(${widthPercentage}% - 8px)`;
};

// Calculate left position for overlapping activities
const calculateActivityLeft = (activity, dayActivities) => {
  const overlappingActivities = findOverlappingActivities(activity, dayActivities);
  
  // If not overlapping, center align
  if (overlappingActivities.length <= 1) {
    return '4px';
  }
  
  // Position horizontally based on position in overlap group
  const positionIndex = overlappingActivities.findIndex(a => a.id === activity.id);
  const widthPercentage = 100 / overlappingActivities.length;
  
  return `calc(${positionIndex * widthPercentage}% + 4px)`;
};
...
/* ---------- SECTION 4: FORMATTING AND DISPLAY HELPERS ---------- */
// Format hour display (e.g., "10:00")
function formatHour(hour) {
  return `${hour}:00`;
}

// Format activity time display (e.g., "16:00 - 17:00")
...
// Format duration for display in tooltip
...
// Format verification date (e.g., "15 Maret 2025, 14:30")
...
// Get CSS class for activity based on type and state
