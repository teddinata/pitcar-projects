<!-- src/components/content/ContentTasksTimeline.vue - FIXED -->
<template>
        ...
        <div class="flex items-center gap-2">
          ...
          <div v-if="hasActiveFilters" class="flex items-center gap-2">
            ...
            <div class="flex gap-1">
              <div
                v-if="contentTypeFilter"
                class="bg-red-50 border border-red-200 text-red-700 text-xs rounded-full px-2 py-1 flex items-center"
              >
                <span>{{ formatContentType(contentTypeFilter) }}</span>
                <XMarkIcon
                  @click="$emit('clearFilter', 'content_type')"
              ...
              <div
                ...
                class="bg-red-50 border border-red-200 text-red-700 text-xs rounded-full px-2 py-1 flex items-center"
              >
                <span>{{ getProjectName(projectFilter) }}</span>
                <XMarkIcon
                  @click="$emit('clearFilter', 'project_id')"
                  class="ml-1 h-3 w-3 cursor-pointer hover:text-red-800"
                />
              </div>
            ...
            <div class="day-row">
              <div
                ...
                :class="[
                  ...
                  !timeUnit.showLabel ? 'no-label' : '',
                ]"
                :style="{ width: `${dayWidth}px` }"
                :title="timeUnit.fullLabel"
              >
                <span v-if="timeUnit.showLabel" class="text-xs text-gray-500">{{
                  timeUnit.label
                }}</span>
              </div>
            </div>
            ...
            <div class="projects-list" ref="projectsList">
              ...
              >
                  ...
                  <div class="project-row-content">
                    ...
                    <div class="project-name-container">
                      ...
                      <div class="project-meta text-xs text-gray-500">
                        {{ project.children?.length || 0 }} tasks
                ...
                <template v-if="project.expanded">
                  <div
                    v-for="task in project.children"
                    :key="task.id"
                    :id="`task-row-${task.id}`"
                    class="task-row"
                    :data-row-id="task.id"
...
// FIXED: Define dayWidth as a computed property FIRST
const dayWidth = computed(() => {
  const screenWidth = window.innerWidth || 1024;
  let baseDayWidth = 30; // Default
  ...
  // Apply zoom with better minimum constraints
  const minWidth = screenWidth < 640 ? 12 : 15;
  return Math.max(minWidth, baseDayWidth * zoomLevel.value);
});

// FIXED: Define todayPosition as computed property
const todayPosition = computed(() => {
  try {
    ...
    return Math.max(0, daysFromViewStart * dayWidth.value);
  } catch (error) {
...
const hasActiveFilters = computed(() => {
  return (
    props.projectFilter ||
    props.contentTypeFilter ||
    props.startDateFilter ||
    props.endDateFilter ||
    props.assignedToFilter
  );
});
...
// Enhanced responsive time units calculation
const timeUnits = computed(() => {
  try {
    ...
    const currentDayWidth = dayWidth.value;

    const days = eachDayOfInterval({ start: rangeStart, end: rangeEnd });

    // Major time units (months) with proper width calculation
    const majorUnits = [];
    ...
    monthsMap.forEach((monthData) => {
      majorUnits.push({
        ...monthData,
        width: monthData.days.length * currentDayWidth,
      });
    });

    // Enhanced responsive minor time units
    const minorUnits = days.map((day, index) => {
      const shouldShowLabel = () => {
        if (currentDayWidth >= 40) return true; // Always show if wide enough
        if (currentDayWidth >= 30) return index % 2 === 0; // Every other day
        if (currentDayWidth >= 20) return index % 3 === 0; // Every 3rd day
        if (currentDayWidth >= 15) return index % 7 === 0; // Weekly
        return index % 14 === 0; // Bi-weekly on very small
      };

      return {
        key: format(day, "yyyy-MM-dd"),
        label: shouldShowLabel() ? format(day, "d") : "",
        fullLabel: format(day, "MMM d, yyyy"),
        date: day,
        isWeekend: isWeekend(day),
        isToday: isToday(day),
        monthLabel: format(day, "MMM"),
        position: index * currentDayWidth,
...
// Methods for timeline functionality
function fetchTasksTimeline() {
  loading.value = true;
  ...
  if (props.projectFilter) {
    params.params.project_id = parseInt(props.projectFilter);
  }

  if (props.contentTypeFilter) {
    params.params.content_type = props.contentTypeFilter;
  }

  if (props.assignedToFilter) {
    params.params.assigned_to = parseInt(props.assignedToFilter);
  }
  ...
  console.log(
    "Fetching Tasks Timeline data with params:",
    JSON.stringify(params)
  );

  apiClient
    .post("/web/v2/content/tasks/timeline", params)
    .then((response) => {
      if (response.data.result?.status === "success") {
        console.log("Tasks Timeline data fetched successfully");
        ...
        console.error(
          "Error fetching Tasks Timeline data:",
          response.data.result?.message
        );
      }
    })
    .catch((err) => {
      console.error("API error fetching tasks timeline data:", err);
    })
    .finally(() => {
      loading.value = false;
    });
...
function getTimeUnitPosition(timeUnit) {
  ...
  return daysFromViewStart * dayWidth.value;
}
...
function getBarPosition(item) {
  ...
  try {
    ...
    const leftPosition = daysFromViewStart * dayWidth.value;
    ...
    const width = Math.max(dayWidth.value * 0.8, itemDuration * dayWidth.value);
...
// Enhanced zoom methods with responsive constraints
function increaseZoom() {
  const maxZoom = window.innerWidth < 768 ? 2 : maxZoomLevel.value;
...
function decreaseZoom() {
  const minZoom = window.innerWidth < 768 ? 0.3 : minZoomLevel.value;
...
function formatContentType(contentType) {
  if (!contentType) return "All Types";
...
function getProjectTooltip(project) {
  ...
  tooltip += `Tasks: ${project.children?.length || 0}`;
...
// Lifecycle hooks with enhanced responsive handling
let resizeObserver = null;
let handleResize = null;

onMounted(() => {
  console.log("Content Tasks Timeline component mounted");
  ...
  handleResize = () => {
    ...
    resizeTimeout = setTimeout(() => {
      // Force reactivity update - this will trigger dayWidth computed
      const currentZoom = zoomLevel.value;
      zoomLevel.value = currentZoom + 0.001; // Tiny change to trigger reactivity
      zoomLevel.value = currentZoom; // Reset to original
...
watch(
  ...
  (
    [newStart, newEnd, newProject, newType, newAssigned],
    [oldStart, oldEnd, oldProject, oldType, oldAssigned]
  ) => {
    console.log("Tasks Timeline filter change detected");
...
<!-- Responsive styles for tasks timeline -->
<style scoped>
.gantt-main {
...
/* Prevent text selection in timeline */
.gantt-headers,
.timeline-chart,
.projects-list {
  user-select: none;
