<!-- src/views/dashboard/BAUDashboard.vue -->
<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <main class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="bg-white overflow-hidden shadow rounded-lg transition-all duration-200 hover:shadow-md">
          ...
          <div class="bg-gray-50 px-5 py-3">
            <div class="text-sm">
              <router-link to="/bau/activities" class="font-medium text-red-600 hover:text-red-500 transition-colors duration-200">
      ...
      <div class="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-2">
        <!-- BAU Activity by Type -->
        <div class="bg-white rounded-lg shadow transition-all duration-200 hover:shadow-md">
          <ActivityTypeChart 
            v-if="activitiesByType && Object.keys(activitiesByType).length > 0"
            :data="activitiesByType" 
            :isActivitiesCount="true"
          />
          <div v-else class="p-6">
            <h3 class="text-lg font-medium leading-6 text-gray-900">Aktivitas berdasarkan Jenis</h3>
            <div class="mt-2 h-64 flex items-center justify-center bg-gray-100 rounded-lg">
              <p v-if="loading" class="text-gray-500">Memuat data grafik...</p>
              <p v-else class="text-gray-500">Tidak ada data aktivitas tersedia</p>
            </div>
          </div>
        </div>

        <!-- BAU Realization Trend -->
        <div class="bg-white rounded-lg shadow transition-all duration-200 hover:shadow-md">
          <RealizationTrendChart 
            v-if="dailyData && dailyData.length > 0"
            :data="dailyData" 
          />
          <div v-else class="p-6">
            ...
            <div class="mt-2 h-64 flex items-center justify-center bg-gray-100 rounded-lg">
              <p v-if="loading" class="text-gray-500">Memuat data grafik...</p>
              <p v-else class="text-gray-500">Tidak ada data tren tersedia</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="mt-8">
        <div class="bg-white shadow rounded-lg overflow-hidden transition-all duration-200 hover:shadow-md">
          <div class="px-6 py-5 border-b border-gray-200 flex justify-between items-center">
            ...
            <router-link to="/bau/calendar" class="text-sm font-medium text-red-600 hover:text-red-500 transition-colors duration-200">
              Lihat kalender
            </router-link>
          </div>
          ...
          <div class="bg-gray-50 px-6 py-3 flex justify-center">
            <router-link to="/bau/activities" class="text-sm font-medium text-red-600 hover:text-red-500 transition-colors duration-200">
              Lihat semua aktivitas
            </router-link>
          </div>
        </div>
    ...
    >
      <div class="space-y-4">
        <div class="h-64">
          <BauVsProjectChart :bauHours="bauHours" :projectHours="projectHours" />
        </div>
...
import ActivityTypeChart from '@/components/charts/ActivityTypeChart.vue'
import RealizationTrendChart from '@/components/charts/RealizationTrendChart.vue' // Ganti nama komponen chart
import BauVsProjectChart from '@/components/charts/BauVsProjectChart.vue'
...
const fetchDashboardData = async () => {
  try {
    ...
    if (response.data.result?.status === 'success') {
      ...
      dailyData.value = processDailyDataForRealization(reportData.daily_data)
...
// Process daily data for realization chart
const processDailyDataForRealization = (dailyData) => {
  if (!dailyData) return []
