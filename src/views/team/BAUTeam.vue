<!-- src/views/dashboard/TeamBAUTeamView.vue -->
<template>
...
const fetchTeamData = async () => {
  try {
    ...
    // Call BAU report API for team module
    ...
    if (response.data.result?.status === 'success') {
      ...
      teamMembers.value = reportData.creators.map(creator => {
        ...
        return {
          ...
          bauDays: (new Set(activities.map(a => a.date.split('T')[0]))).size,
          ...
          totalHours: activities.reduce((sum, a) => sum + (a.hours_spent || 0), 0)
        }
      })
